...
import com.lee.hof.common.util.LogUtil;
...
public class UserStatisticServiceImpl extends ServiceImpl<UserStatisticMapper, UserStatistic> implements UserStatisticService {
    ...
    @Override
    public UserStatisticVO getMine() {
        ...
        List<Post> myPosts = postMapper.selectList(new QueryWrapper<Post>().select("id").eq("author_id",UserContext.getUserId()));
        Set<Long> myPostIds = myPosts.stream().map(Post::getId).collect(Collectors.toSet());
        List<Comment> myComments = commentMapper.selectList(new QueryWrapper<Comment>().select("id").eq("user_id",UserContext.getUserId()));
        Set<Long> myCommentIds = myComments.stream().map(Comment::getId).collect(Collectors.toSet());

        if(!myCommentIds.isEmpty()){
            LogUtil.info(JSONObject.toJSONString(myCommentIds));
        ...
        if(!myPostIds.isEmpty()){
            LogUtil.info(JSONObject.toJSONString(myPostIds));
            List<Like> likes = likeMapper.selectList(new QueryWrapper<Like>()
                    .eq("status", 0 )
        ...
        if(!myPostIds.isEmpty()){
            conditions.and(commentQueryWrapper -> commentQueryWrapper.eq("reply_to_user_id", UserContext.getUserId()).or().in("entity_id", myPostIds));
        }else{
            conditions.and(commentQueryWrapper -> commentQueryWrapper.eq("reply_to_user_id", UserContext.getUserId()));
        }
    ...
    private UserStatistic getMyStatic(){
        return getBaseMapper().selectOne(new QueryWrapper<UserStatistic>().eq("create_by", UserContext.getUserId()));
    }
